...
int PDU_Decode(const char* Src)
{
	...
	if (UDHI == 0)  //无用户数据头
	{
		...
		if (pdu_message.TP_DCS == GSM_7BIT) //7 bit解码
		{
			...
			//特殊扩展处理
			n_dstlength = PDUdcode_7bit(buf, extend_buf, n_dstlength);						// 转换到TP-DU 
		...
		if (pdu_message.UDH_headlength == 0x05)    //只需判断一个标识 取出前6个字节
		{
			...
			if (UDH_number == 0x01)
			{
				if (pdu_message.TP_DCS == GSM_7BIT && solo_flag == 1)     //处理单条
				{
					...
					//扩展修改
					n_dstlength = PDUdcode_7bit(buf, &extend_buf[0], n_dstlength);
					extend_check(extend_buf, pdu_message.TP_UD, n_dstlength);           // 7bit 扩展解码函数
				...
				/*	n_dstlength = PDUdcode_7bit(buf, pdu_long[1].TP_UD, n_dstlength);	*/
					...
					//扩展修改
					n_dstlength = PDUdcode_7bit(buf, extend_buf, n_dstlength);
					extend_check(extend_buf, pdu_long[1].TP_UD, n_dstlength);           // 7bit 扩展解码函数
				}
				else if (pdu_message.TP_DCS == GSM_UCS2)  //Unicode 解码
				{
					tmp = pdu_message.TP_UDL - 6;
			...
			else if (decode_num <= UDH_cnt)
			{
				if (pdu_message.TP_DCS == GSM_7BIT) //00
				{
					...
					//扩展修改
					n_dstlength = PDUdcode_7bit(buf, extend_buf, n_dstlength);   //buf的值是没有问题的
					extend_check(extend_buf, pdu_long[UDH_number].TP_UD, n_dstlength);           // 7bit 扩展解码函数
				}
