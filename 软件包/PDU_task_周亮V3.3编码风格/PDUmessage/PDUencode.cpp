...
*	函数功能：	7bit编码
*
*	入口参数：	src 原字符串指针  n_srclength 长度
...
/*
************************************************************
*	函数名称：	PDU_Encode_longmessage_NO1
...
*	说明：  encode_mode= 1 不带时间戳 encode_mode = 2 带时间戳
************************************************************
*/
...
/*
************************************************************
*	函数名称：	PDU_Encode_longmessage_NO2
...
int PDU_Encode_longmessage_NO2(char* Dst)              //超过字符的长短信拆分编码 不带时间戳
{
	...
	if (pdu_message.In_TP_DCS == GSM_7BIT)
	{
		...
		if (encode_num == Cnt - 1)                       //未处理到最后一条子短信
		{
			...
			n_length = PDUencode_7bit(Temp, buf, left + 8);
			n_dstlength += PDU_BytestoString(&buf[6], &Dst[n_dstlength], n_length -7);
...
************************************************************
*/
int extend_change(char *src,int after_num,int sp_num)
{
	const char extend[9] = { '^','{','}','\\','[','~',']','|','�' };
	const char extend_value[9] = {0x14,0x28,0x29,0x2F,0x3C,0x3D,0x3E,0x40,0x65};

	int count = after_num - sp_num;
	while (0!= sp_num)
	{
		src[count - 1 + sp_num] = src[count - 1];
		for (int i =0;i<9;i++)
		{
			if (src[count - 1 + sp_num] == extend[i])
			{
				src[count - 1 + sp_num - 1] = 0x1B;
				src[count - 1 + sp_num] = extend_value[i];
				sp_num--;
				break;
			}
		}
...
************************************************************
*/
int extend_change2(char *src, char *dst)
{
	const char extend[9] = { '^','{','}','\\','[','~',']','|','�' };
	const char extend_value[9] = {0x14,0x28,0x29,0x2F,0x3C,0x3D,0x3E,0x40,0x65 };
