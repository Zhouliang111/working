...
实现的功能：1.7bit解码，8bit解码，Unicode中文解码
			...
			3.超长短信的编码拆分：实现了汉字超过70个字（140个字节）的拆分 汉字拆分和8bit实现
			4.超长短信解码基本实现
...
struct pdu pdu_message;     //定义结构体变量
char messageok[1024] = ""; //参考解析
char PDU_message[1024];
int head_flag = 0;    //定义符合标识的标志位
int head_flag1 = 0;   //定义两位标识的标志位
int nUDI_flag = 0;    //定义无用户信息头标志位
int solo_flag = 0;           //定义子短信的结束标志
int Decode_flag = 0;
int Encode_flag = 0;
...
void pdu_show()
{
	...
	if (solo_flag == 1)                      //有用户头单条的情况
	{
		...
		{
			for (int i = 0 ; i < pdu_message.TP_UDL/2; i++)    //8bit和中文跳过头打印
			{
				printf("%c", pdu_message.TP_UD[i]);
			}
		}
	...
	if (head_flag == 1)        //标识匹配继续输出之后的短消息
	{
		...
		if (num == UDH_cnt)           // 这里需要隔离开长短信之前的短信分片问题
		{
			...
			for (int i = (int)pdu_message.UDH_headlength; i < (int)pdu_message.TP_UDL * 3; i++)
			{
				printf("%c", pdu_message.TP_UD[i]);
			}
		}
