...
int PDU_Decode(const char* Src)
{
	...
	if (UDHI == 0)  //无用户数据头
	{
		...
		else if (pdu_message.TP_DCS == GSM_UCS2)  //Unicode 解码
		{
			...
			n_dstlength = PDUdcode_Unicode(buf, pdu_message.TP_UD, n_dstlength);     //短信内容提取
		}
		else               //8 bit 解码
		{
			n_dstlength = PDU_StringtoBytes(Src, buf, tmp * 2);                     //格式转换
		...
		if (pdu_message.UDH_headlength == 0x05)    //只需判断一个标识 取出前6个字节
		{
			...
			if (UDH_number == 0x01)
			{
				if (pdu_message.TP_DCS == GSM_7BIT && solo_flag == 1)
				{
					...
					n_dstlength = PDUdcode_7bit(buf, &pdu_message.TP_UD[0], n_dstlength);						// 转换到TP-DU
				}
				else if (pdu_message.TP_DCS == GSM_7BIT) //00
				{
					...
					Src -= 10;                                      //回到用户数据信息的位置
					n_dstlength = PDU_StringtoBytes(Src, buf, tmp & 7 ? (int)tmp * 7 / 4 + 2 : (int)tmp * 7 / 4);	// 格式转换
					n_dstlength = PDUdcode_7bit(buf, pdu_long[1].TP_UD, n_dstlength);	// 转换到TP-DU
				...
				else if (pdu_message.TP_DCS == GSM_UCS2)  //Unicode 解码
				{
					...
					Src += 2;
					n_dstlength = PDU_StringtoBytes(Src, buf, tmp * 2);            //格式转换
					n_dstlength = PDUdcode_Unicode(buf, pdu_long[1].TP_UD, n_dstlength);     //短信内容提取
				...
				else               //8 bit 解码
				{
					...
					Src += 2;
					n_dstlength = PDU_StringtoBytes(Src, buf, tmp * 2);                     //格式转换
					n_dstlength = PDUdcode_8bit(buf, pdu_long[1].TP_UD, n_dstlength);   //短信内容提取
				}
			}
			else if (num <= UDH_cnt)
			{
				if (pdu_message.TP_DCS == GSM_7BIT) //00
				{
					...
					Src -= 10;                                      //回到用户数据信息的位置
					n_dstlength = PDU_StringtoBytes(Src, buf, tmp & 7 ? (int)tmp * 7 / 4 + 2 : (int)tmp * 7 / 4);	// 格式转换
					n_dstlength = PDUdcode_7bit(buf, pdu_long[UDH_number].TP_UD, n_dstlength);	// 转换到TP-DU
				...
				else if (pdu_message.TP_DCS == GSM_UCS2)  //Unicode 解码
				{
					...
					Src += 2;
					n_dstlength = PDU_StringtoBytes(Src, buf, tmp * 2);            //格式转换
					n_dstlength = PDUdcode_Unicode(buf, pdu_long[UDH_number].TP_UD, n_dstlength);     //短信内容提取
				...
				else               //8 bit 解码
				{
					...
					Src += 2;
					n_dstlength = PDU_StringtoBytes(Src, buf, tmp * 2);                     //格式转换
					n_dstlength = PDUdcode_8bit(buf, pdu_long[UDH_number].TP_UD, n_dstlength);   //短信内容提取
